...
class NeuralNetworkBN:
    ...
    def __init__(self, context_size:int = 3, hidden_layer_neurons: int = 100, letter_embedding_dimensions: int = 2, print_flag=True, generator_seed=2147483647):
        ...
        # self.b2 = torch.randn(27,generator=self.g) # Commenting out as we are using batch normalization and bnbias is used instead.
        ...
        self.params = [self.c,self.w1,self.w2,self.b2, self.bngain, self.bnbias]
    ...
    def generate_name(self):
        ...
        while predicted_idx!=0:
            hpreact = (self.c[torch.Tensor(context_idx).int()].view(-1,self.context_size*self.letter_embedding_dimensions) @ self.w1) 
    ...
    def calculate_loss(self, X,Y):
        return F.cross_entropy((self.c[X].view(-1,self.context_size*self.letter_embedding_dimensions) @ self.w1 ).tanh() @ self.w2 + self.b2,Y)
    ...
    def train(self, X,Y, training_params: TrainingParams=TrainingParams(iterations=1000, batch_size=30, learning_rate=lambda x: 0.1)):
        ...
        for i in range(training_params.iterations):
            ...
            # LINEAR LAYER
            hpreact = (self.c[X[minibatch]].view(-1, self.context_size * self.letter_embedding_dimensions) @ self.w1)   
